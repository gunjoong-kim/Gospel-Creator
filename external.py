...
def densify_with_learnable_mask(params, variables, optimizer, i):
    if i <= 5000:
        variables = accumulate_mean2d_gradient(variables)
        grad_thresh = 0.0002
        if (i >= 500) and (i % 100 == 0):
            ...
            n = 2  # number to split into
            new_params = {
                k: v[to_split].repeat(n, 1, 1) if k == 'sh_coeffs' or k == 'rgb_colors' else v[to_split].repeat(n, 1)
            ...
            remove_threshold = 0.25 if i == 5000 else 0.005
            #to_remove = (torch.sigmoid(params['logit_opacities']) < remove_threshold).squeeze()
            to_remove = torch.logical_or((torch.sigmoid(params['mask']) <= 0.01).squeeze(), (torch.sigmoid(params['logit_opacities']) < remove_threshold).squeeze())
            if i >= 3000:
                big_points_ws = torch.exp(params['log_scales']).max(dim=1).values > 0.1 * variables['scene_radius']
                to_remove = torch.logical_or(to_remove, big_points_ws)
            params, variables = remove_points(to_remove, params, variables, optimizer)
